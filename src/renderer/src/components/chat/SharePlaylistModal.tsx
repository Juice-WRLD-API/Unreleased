...
import { encodePlaylistShare } from '../../lib/chatShare'
...
export default function SharePlaylistModal({ playlist, onClose }: Props): JSX.Element {
  ...
  const share = async (room: RoomRef): Promise<void> => {
    ...
    setError(null)
    try {
      await send(room, { text: encodePlaylistShare(playlist), files: [] })
      ...
      >
        <header className="flex items-start gap-3 px-5 pt-5 pb-3">
          <div className="flex-1 min-w-0">
            <h2 className="text-lg font-bold text-text-primary">Share playlist</h2>
            <p className="text-sm text-text-muted mt-0.5 truncate">{playlist.name}</p>
          </div>
