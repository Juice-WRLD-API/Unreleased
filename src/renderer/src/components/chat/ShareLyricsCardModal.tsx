...
// Unlike song/playlist shares (a small JSON payload inlined into the message
// text, see chatShare.ts), a lyrics card is a rasterized PNG - it rides as a
// normal chat file attachment instead, going through the same upload/encrypt
// path (chatStore.send's `files`) as any other image someone drops into chat.
export default function ShareLyricsCardModal({ file, title, artist, onClose }: Props): JSX.Element {
  ...
  const share = async (room: RoomRef): Promise<void> => {
    ...
    setError(null)
    try {
      await send(room, { text: '', files: [file] })
  ...
  return createPortal(
    <div className="fixed inset-0 z-[180] flex items-end md:items-center justify-center md:p-4" onMouseDown={onClose}>
